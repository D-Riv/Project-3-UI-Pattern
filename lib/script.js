    ...
    fetch(heroUrl + "/" + e.target.value)
      ...
      .then((heroData) => {
        ...
        let searchResults = heroData.results.map((item) => {
          ...
          const birthPlace = Object.values(item.biography)[4];
          const firstAppearance = Object.values(item.biography)[5];
          const comicPublisher = Object.values(item.biography)[6];
